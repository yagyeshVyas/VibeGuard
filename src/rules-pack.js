...
const extraSecretRules = [
  ...
  { id: 'secret.huggingface-token', severity: 'critical', re: /\bhf_[A-Za-z0-9]{30,}\b/, cwe: 'CWE-798', owasp: 'A02', message: 'Hugging Face API token exposed.', fix: 'Move to process.env.HUGGINGFACE_TOKEN and rotate the token in Hugging Face settings.' },
  { id: 'secret.docker-pat', severity: 'high', re: /\bdpc_[A-Za-z0-9]{36}\b/, cwe: 'CWE-798', owasp: 'A02', message: 'Docker Hub personal access token exposed.', fix: 'Move to process.env.DOCKER_TOKEN and rotate the token in Docker Hub settings.' },
  { id: 'secret.figma-token', severity: 'high', re: /\bfigd_[A-Za-z0-9_-]{40,}\b/, cwe: 'CWE-798', owasp: 'A02', message: 'Figma API token exposed.', fix: 'Move to process.env.FIGMA_TOKEN and rotate the token in Figma account settings.' },
  { id: 'secret.shopify-admin-token', severity: 'high', re: /\bshpat_[A-Za-z0-9]{32,}\b/, cwe: 'CWE-798', owasp: 'A02', message: 'Shopify admin API access token exposed — full store access.', fix: 'Move to process.env.SHOPIFY_ADMIN_TOKEN and rotate the token in Shopify admin.' },
  { id: 'secret.postman-api-key', severity: 'high', re: /\bPMAK-[A-Za-z0-9-]{60,}\b/, cwe: 'CWE-798', owasp: 'A02', message: 'Postman API key exposed.', fix: 'Move to process.env.POSTMAN_API_KEY and rotate the key in Postman settings.' },
];
...
const amazingRules = [
  ...
  // =========================================================================
  // SSTI + ENV LEAK RULES — template injection and process.env exposure
  // =========================================================================
  { id: 'code.ssti-ejs', severity: 'critical', re: /ejs\.(?:render|renderFile)\s*\([^)]*(?:req|user|input|body|query|params)\./, skipComments: true, confidence: 'high', message: 'EJS template rendered with user input — server-side template injection (RCE)', cwe: 'CWE-94', owasp: 'A03', fix: 'Pass user input as data context, never as the template string: ejs.render(template, { data: userInput }).' },
  { id: 'code.ssti-handlebars', severity: 'critical', re: /Handlebars\.(?:compile|precompile)\s*\([^)]*(?:req|user|input|body|query|params)\./, skipComments: true, confidence: 'high', message: 'Handlebars template compiled from user input — SSTI', cwe: 'CWE-94', owasp: 'A03', fix: 'Compile templates at startup, never from user input. Pass user data as context.' },
  { id: 'code.ssti-nunjucks', severity: 'critical', re: /nunjucks\.(?:renderString|compile)\s*\([^)]*(?:req|user|input|body|query|params)\./, skipComments: true, confidence: 'high', message: 'Nunjucks renderString with user input — SSTI', cwe: 'CWE-94', owasp: 'A03', fix: 'Use nunjucks.render(template, context). Never renderString with user input as the template.' },
  { id: 'code.ssti-pug', severity: 'critical', re: /pug\.(?:render|compile|compileClient)\s*\([^)]*(?:req|user|input|body|query|params)\./, skipComments: true, confidence: 'high', message: 'Pug template compiled from user input — SSTI', cwe: 'CWE-94', owasp: 'A03', fix: 'Compile templates once. Pass user input via locals object, never as the template source.' },
  { id: 'code.ssti-mustache', severity: 'critical', re: /Mustache\.(?:render|parse)\s*\([^)]*(?:req|user|input|body|query|params)\.\s*[,)]/, skipComments: true, confidence: 'medium', message: 'Mustache template rendered from user input — possible SSTI', cwe: 'CWE-94', owasp: 'A03', fix: 'Render fixed templates. Pass user input via the data object, not as the template string.' },
  { id: 'code.json-stringify-env', severity: 'critical', re: /JSON\.stringify\s*\(\s*process\.env(?:\s*\.\s*\w+)?\s*\)/, skipComments: true, confidence: 'high', message: 'JSON.stringify(process.env) — leaks ALL environment variables (secrets, DB credentials) to the client', cwe: 'CWE-200', owasp: 'A02', fix: 'Never serialize process.env. Select specific safe values: const config = { API_URL: process.env.API_URL }.' },
  { id: 'code.process-env-to-res', severity: 'critical', re: /res\.(?:json|send|render|write)\s*\(\s*process\.env(?:\s*\.\s*\w+)?\s*\)/, skipComments: true, confidence: 'high', message: 'process.env sent directly in response — leaks all secrets to client', cwe: 'CWE-200', owasp: 'A02', fix: 'Never send process.env to the client. Build a safe config object with only public values.' },
];
