...
const aiSecurityRules = [
  ...
  },
  {
    ...
    re: /model\s*:\s*req\.(?:body|query|params)\./i,
    skipComments: true,
  ...
  },
  {
    ...
    re: /(?:agent|tool|function)\s*[:=]\s*[^}]*(?:deploy|publish|release|ship)/i,
    skipComments: true,
  ...
  },
  {
    ...
    re: /(?:agent|tool|function)\s*[:=]\s*[^}]*(?:process\.env|environment|secrets|\.env)/i,
    skipComments: true,
...
const csharpRules = [
  { id: 'csharp.sql-injection', severity: 'high', re: /SqlCommand\s*\(\s*["'].*\+\s*\w/, message: 'C# SqlCommand with string concatenation — SQL injection', cwe: 'CWE-89', owasp: 'A03', fix: 'Use SqlParameter: cmd.Parameters.AddWithValue("@id", id).' },
...
const aiSecurityRulesExtra = [
  ...
  { id: 'ai.tool-result-injection', severity: 'high', re: /(?:response|result|output)\s*(?:\.text|\.content|\.body|\.json)\s*(?:\+|\`\$\{)/, message: 'Tool result concatenated into prompt — indirect prompt injection', cwe: 'CWE-94', owasp: 'A03', fix: 'Escape tool results before including in prompt. Use structured parsing.' },
...
const powerRulesExtra = [
  ...
  { id: 'ai.no-input-sanitization', severity: 'high', re: /(?:messages|prompt|input)\s*[:=]\s*req\.(?:body|query|params)\.(?:prompt|message|input|text)/i, confidence: 'low', message: 'AI input from user without sanitization — prompt injection', cwe: 'CWE-20', owasp: 'A03', fix: 'Sanitize user input before passing to LLM. Strip injection patterns.' },
  ...
  { id: 'ai.no-rate-limit', severity: 'medium', re: /(?:openai|anthropic|gemini|llama)\.(?:completions|chat|messages|generate)/i, confidence: 'low', message: 'LLM API call without rate limiting — cost abuse risk', cwe: 'CWE-770', owasp: 'A04', fix: 'Add rate limiting on AI endpoints. Track and limit per-user costs.' },
