...
function makeFinding(rule, opts) {
  return {
    ...
    // Confidence: 'high'  = mechanical match, safe to act on.
    //             'medium'= strong heuristic, glance before acting.
    //             'low'   = "review this" — a hint, not a verdict.
    confidence: opts.confidence || rule.confidence || 'high',
...
const robustnessRules = [
  ...
  },
  {
    ...
    re: /\.(?:find|findOne|findOneAndUpdate|findOneAndDelete|updateOne|updateMany|deleteOne|deleteMany|count|countDocuments)\s*\(\s*req\.(?:body|query|params)\s*[),]|\$where\s*:/,
    skipComments: true,
...
const injectionRules = [
  {
    ...
    re: /\b(?:_\.)?(?:merge|mergeWith|defaultsDeep|extend|set|setWith)\s*\([^)]*\breq\.(?:body|query|params)\b|deepmerge\s*\([^)]*\breq\.(?:body|query|params)\b/i,
    skipComments: true,
