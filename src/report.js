...
function renderHuman(result) {
  ...
  lines.push('');
  // Baseline suppression notice
  if (result.baselineSuppressed && result.baselineSuppressed > 0) {
    lines.push(`${C.dim}${result.baselineSuppressed} known finding${result.baselineSuppressed === 1 ? '' : 's'} suppressed (baseline). Run with --all to see.${C.reset}`);
  }
  // Fix hint: show how many issues are auto-fixable
  if (result.findings.length > 0) {
    const fixable = result.findings.filter(f => f.fix && f.ruleId).length;
    if (fixable > 0) {
      lines.push(`${C.dim}Run ${C.bold}vibeguard fix${C.reset}${C.dim} to auto-fix ${fixable} issue${fixable === 1 ? '' : 's'}${C.reset}`);
    }
  }
  lines.push(
