...
function collectRefs(node, out) {
  ...
  if (node.type === 'MemberExpression') { const r = ast.rootName(node); if (r) out.add(r); return out; }
  for (const k in node) {
...
function analyzeFunction(node, walk, resolveCallee) {
  ...
  const assigns = [];
  walk.simple(node, {
    VariableDeclarator(n) { if (n.id && n.id.type === 'Identifier' && n.init) assigns.push({ t: n.id.name, e: n.init }); },
    AssignmentExpression(n) { if (n.left && n.left.type === 'Identifier') assigns.push({ t: n.left.name, e: n.right }); },
  });
  for (let pass = 0; pass < 5; pass++) {
    let changed = false;
    for (const a of assigns) {
      const slots = new Set();
      for (const r of collectRefs(a.e)) { const s = taintOf.get(r); if (s) for (const x of s) slots.add(x); }
      if (slots.size) {
  ...
  const slotsFor = (n) => {
    const s = new Set();
    for (const r of collectRefs(n)) { const t = taintOf.get(r); if (t) for (const x of t) s.add(x); }
    return s;
  };
  ...
  walk.simple(node, {
    ...
    ReturnStatement(r) { for (const slot of slotsFor(r.argument)) returnsSlots.add(slot); },
  });
