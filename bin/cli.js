...
const VALUE_FLAGS = new Set(['fail-on', 'scope', 'focus', 'model', 'since', 'base', 'o', 'output', 'file', 'types']);
...
VibeGuard — catch common security problems in vibe-coded apps.

Usage:
  vibeguard scan [dir]        Scan a project and report findings by severity.
  vibeguard fix [dir]         List concrete fixes + a copy-paste prompt for your AI.
  vibeguard verify [dir]      Re-scan and report which issues are now resolved.
  vibeguard install           Wire the MCP server into every detected AI client.
  vibeguard mcp               Run the MCP server on stdio (used by AI clients).
  vibeguard rules             List every detection rule.
  vibeguard explain <ruleId>  Show details + fix for one rule.
  vibeguard doctor [dir]      Audit AI-tool host config (hooks, base URL, MCP).
  vibeguard install-hook      Install the git pre-commit hook (blocks on CRITICAL).
  vibeguard rollback [dir]    Undo the last 'fix --apply' from its snapshot.
  vibeguard watch [dir]       Re-scan on every file change (live feedback).
  vibeguard history [dir]     Scan git history for secrets (--all for full depth).
  vibeguard url <url>         Scan a LIVE site's HTTP response headers/cookies.
  vibeguard badge [dir]       Award a badge SVG — only if zero critical & high.
  vibeguard init-ci [dir]     Write a GitHub Actions workflow (SARIF + gate).
  vibeguard compliance [dir]   Map findings to SOC2/PCI/HIPAA/GDPR/ISO27001/EUAIAct.
  vibeguard cve [dir]         Query OSV.dev for known dependency vulnerabilities.
  vibeguard slopsquat [dir]    Check for hallucinated/fake npm packages.
  vibeguard secure-prompt <p> Analyze an AI prompt for security risks BEFORE code.
  vibeguard trace [dir]        Trace findings to AI prompts + behavior analysis.
  vibeguard repl [dir]        Interactive REPL: inspect, solve, ignore findings.
  vibeguard auth-coverage [dir] Enumerate routes → middleware → auth guards.
  vibeguard diff [dir]        Compare current scan vs baseline (resolved/new/remaining).
  vibeguard deep-scan [dir]    Full audit: scan + CVE + auth + behavior + traces.
  vibeguard html [dir]        Generate a self-contained HTML security report.
  vibeguard inject-rules [dir] Inject security rules into CLAUDE.md/.cursorrules/GEMINI.md.
  vibeguard bench [dir]       Run a benchmark: files/sec, rules, findings, time.
  vibeguard install-hook-post  Install PostToolUse hook (auto-scans after AI edits).
  vibeguard uninstall-hook-post  Remove the PostToolUse hook.
  vibeguard hook-post-edit    Internal: scan a file after AI edit (called by hook).
  vibeguard risk-score [dir]   Quick weighted risk score (0-100) with risk level.
  vibeguard trend [dir]        Compare current scan against baseline — show resolved/new.
  vibeguard why <ruleId>        Explain why a rule fired and how to fix it.
  vibeguard config-check [dir]  Validate VibeGuard configuration and show status.
  vibeguard watch [dir]         Watch for file changes and auto-scan on save.
  vibeguard dashboard [dir]     Beautiful terminal dashboard with visual charts.
  vibeguard pr-comment [dir]    Output GitHub PR comment format (markdown).
  vibeguard preset <name>       Apply a rule pack preset (nextjs, django, rails, aws, etc.).
  vibeguard privacy-audit [dir]  Inventory all PII collection, storage, and transmission.
  vibeguard net-audit [dir]     Map every outbound HTTP call. Flag unknown endpoints.
  vibeguard ai-guard [dir]      Detect user data sent to AI APIs without redaction.
  vibeguard privacy-policy [dir]  Auto-generate a privacy policy from code analysis.
  vibeguard csp-generate [dir]  Auto-generate Content-Security-Policy from code.
  vibeguard firewall <prompt>    AI Firewall — inspect a prompt before sending to LLM.
  vibeguard agent-guard [dir]    Show agent constraints (blocked paths, commands, domains).
  vibeguard exfil-check <data>   Check if data contains PII/secrets before sending externally.
  vibeguard dep-firewall <pkg>   Check a package name before npm install (malware/typosquat).
  vibeguard intercept-test [dir]  Test runtime interceptor (simulates jailbroken AI).
  vibeguard output-guard <text>  Sanitize AI response for PII/secrets before showing to user.
  vibeguard env-lock [dir]       Lock environment variables — protect secrets from jailbroken AI.
  vibeguard self-check [dir]     Check if VibeGuard itself has been tampered with.
  vibeguard sandbox <code>       Execute AI-generated code in zero-trust sandbox.
  vibeguard behavior [dir]       Behavioral analysis of AI agent patterns.
  vibeguard supply-firewall [dir] Supply chain audit: lockfile, scripts, licenses.
  vibeguard vault <action>        Secret vault: store, list, log (encrypted in-memory).
  vibeguard audit-trail [action]  Tamper-proof audit trail: show, export, verify.
  vibeguard pre-deploy [dir]     Run ALL 13 security gates before deployment.
                                Use --strict to fail on warnings, --json for CI/CD.
  vibeguard preset --list       List all available presets.
  vibeguard redact "<text>"     Redact personal data (email/SSN/card/phone/…) from text.
  vibeguard detect-pii "<text>" List personal data found in text (no redaction).

Options:
  --json                      Output machine-readable JSON.
  --sarif                     Output SARIF 2.1.0 (for CI / GitHub code scanning).
  --fix-prompt                Print only the paste-back "fix prompt" block.
  --apply                     (fix) Apply safe auto-fixes (snapshotted; default is dry-run).
  --local                     (install) Point clients at this checkout, not npx.
  --baseline                  (scan) Save a baseline for later 'verify'.
  --no-deps                   (scan) Skip the npm/pip dependency audit.
  --deep                      (scan) Fold in semgrep/gitleaks/bandit if installed.
  --new-only                  (scan) Report only findings not in the saved baseline.
  --verify-keys               (scan) Ping providers to confirm found keys are LIVE (opt-in).
  --fail-on <level>           (scan) Exit non-zero only at/above this severity (critical..low/none).
  --precommit                 Exit non-zero only on CRITICAL findings (hook mode).
  --no-summary                Suppress the trailing summary line.
  --patch                     Output findings as a unified diff (git-apply format).
  -o, --output <file>         (html) Output file path for HTML report.
  -h, --help                  Show this help.

Scope: VibeGuard flags high-frequency, well-defined issues (secrets, injection,
misconfig) and helps you fix them. It does NOT prove an app is safe and cannot
catch every logic bug or backdoor.
`);
}
...
function cmdScan(dir, flags) {
  ...
  // --new-only: report only findings not present in the saved baseline.
  if (flags['new-only']) {
...
async function main() {
  ...
  const noDirCmds = new Set(['url', 'rules', 'explain', 'secure-prompt', 'redact', 'detect-pii', 'firewall', 'exfil-check', 'dep-firewall', 'sandbox', 'output-guard', 'vault', 'audit-trail', 'why']);
  ...
  try {
    ...
    else if (cmd === 'install-hook-post') { const r = require('../src/hook').installPostEditHook(dir); process.stdout.write(r.installed ? `Installed PostToolUse hook in ${r.file}\n` : `Hook ${r.reason}\n`); code = 0; }
    else if (cmd === 'uninstall-hook-post') { const r = require('../src/hook').uninstallPostEditHook(dir); process.stdout.write(r.uninstalled ? 'Uninstalled.\n' : `Not found: ${r.reason}\n`); code = 0; }
